...
def decide_attack(result_list, attack_para_list=[], now_time="0", ori_acc=0.9):
    ...
    if previous_acc < ori_acc * 0.4 or previous_para_n + 1 >= len(
        attack_method_list[previous_attack_n][2]
    ):
        strong = (previous_para_n + 1) / len(attack_method_list[previous_attack_n][2])
        out_string = ""
        if strong >= 0.8 and previous_acc >= ori_acc * 0.4:
            out_string = (
                "Your model is very robust on "
                + str(attack_method_list[previous_attack_n][3])
                + "\n"
            )
        elif strong >= 0.5 and previous_acc >= ori_acc * 0.2:
            out_string = (
                "Your model is barely robust on "
                + str(attack_method_list[previous_attack_n][3])
                + "\n"
            )
        else:
            out_string = (
                "Your model is not robust on "
                + str(attack_method_list[previous_attack_n][3])
                + "\n"
            )
        next_para_n = 0
